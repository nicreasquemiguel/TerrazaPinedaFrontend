...
export const REACT_APP_API_URL = 'http://localhost:8000' //localhost 
...
export const googleAuthenticate = (state, code) => async dispatch => {
    if (state && code && !localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post(`${REACT_APP_API_URL}/auth/o/google-oauth2/?${formBody}`, config);
...
export const facebookAuthenticate = (state, code) => async dispatch => {
    if (state && code && !localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post(`${REACT_APP_API_URL}/auth/o/facebook/?${formBody}`, config);
...
export const reset_password = (email) => async dispatch => {
    ...
    try {
        await axios.post(`${REACT_APP_API_URL}/auth/users/reset_password/`, body, config);
